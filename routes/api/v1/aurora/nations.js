...
router.get("/:nationName", async (req, res) => 
{
    ...
    if (cachedNations) {
        ...
        var foundNation = await emc.Aurora.getNation(nationName).then(nation => { return nation })
...
{
    var nationName = req.params.nationName,
        invitableTownsRes = await emc.Aurora.getInvitableTowns(nationName, false).then(towns => { return towns })
